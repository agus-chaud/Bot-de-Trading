...
| 8 | Breadth insuficiente para medir la señal | Medición de señal | **En progreso** |

Hilo conductor: varias de estas complicaciones estaban **encadenadas** — una tapaba a la otra. El 401 de IOL (#1) ocultaba el bug de mapeo (#3), que a su vez quedaba enmascarado por el fallback silencioso a Byma (#4). Y la mezcla de monedas (#6) inflaba un veredicto de señal (#7) que, al limpiarse, dejó al descubierto el problema real de fondo: falta de amplitud del universo (#8). Resolver una capa fue, repetidamente, la condición para ver la siguiente.
...
- **Resolución**: el usuario activó los permisos de API de la cuenta IOL.
- **Estado**: **Resuelto**.
- **Lección**: un `login` exitoso **no implica** permisos de datos. Hay que probar **endpoints reales** (no solo el login) para distinguir "autenticación OK" de "autorización OK".
...
- **Estado**: **Identificado** (corregido en el diagnóstico).
...
- **Estado**: **En progreso** (universo ya ampliado; falta re-correr la medición sobre U2).
