...
10. **Concentracion y factor (ADR-059, prioritario antes de capital real)**: el walk-forward de investigacion mostro que ~60% del capital esta en GGAL+PAMP, mismo factor (riesgo-pais AR) → drawdowns de -25% en selloffs locales. Tres frentes: (a) **bajar concentracion** del largo (mas nombres, menos peso por nombre); (b) **diversificar el factor** subiendo el peso de exposicion global (CEDEARs) frente al equity AR puro; (c) que el **sleeve corto cubra de verdad** (hoy termina flat, no aporta retorno descorrelacionado) o reducir su asignacion.
   ...
   **Estado (2026-06-25)**: el frente (c) se implemento (hedge GLD/WMT, **ADR-062/064**). El intento de ampliar diversificacion **agregando papeles** (experimento D: largo +PG/JNJ, hedge +MCD/PFE) dio resultados corregidos (**ADR-069**): (a) ampliar el **hedge** con MCD/PFE es valido pero **NO supera** a producción C (Calmar 2,175 < 2,316) y no mueve el drawdown del crash global (-16,6%); (b) ampliar el **largo** con PG/JNJ rompía el motor largo porque PG/JNJ no estaban whitelisteados (el largo aborta el ciclo ante símbolo no whitelisteado, `long_term_engine.py:308`) — **ya fixeado** (`whitelist_cedear_D.yaml`); con el largo vivo D da TWR 56,0% / Calmar 2,18, que tampoco supera a C. El "-4,8% de drawdown" inicial era un artefacto del largo apagado.

   **Cimiento de des-riesgo (ADR-071, 2026-06-25)**: el sleeve largo ahora **puede mantener cash** (`allow_cash` + `equity_exposure`, detrás de feature flag apagado por defecto — producción intacta). Una corrida de sanidad con exposición fija 0,70 bajó el drawdown de la diversificada de **-17,1% a -11,6%**. Esto habilita el trailing stop ATR y un des-riesgo por régimen; falta implementar **CUÁNDO** des-riesgar (el trigger de `equity_exposure`), que es el próximo paso. Hallazgo de disponibilidad: el factor **TASAS/BONOS no existe como CEDEAR en BYMA** (TLT/AGG/BND/LQD/IEF/SHY/TIP/HYG/EMB → 404); proxies disponibles: **SH** (inverso S&P, 365d) y **XLU** (utilities, <300d). Los cuatro defensivos (JNJ/MCD/PG/PFE) pasan corr-en-crisis ≤ 0. Proximo paso pre-registrado: un D' con **menos tilt defensivo** (que baje V4/V5 sin matar el upside de los rallies).
