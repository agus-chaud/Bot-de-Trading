...
| Calendario | `calendar.source_of_truth` debe apuntar a un YAML existente con sesiones US y días hábiles AR; `run_paper_live.py` aborta si falta (**ADR-054**). |
...
- **Paper-live**: `run_paper_live.py` **exige** que exista el archivo apuntado por `calendar.source_of_truth` (fail-fast, `exit 1` si falta o está vacío — **ADR-054**). Flag `--no-calendar` solo para tests/diagnóstico.
