...
### Removed
- **Artefactos one-off del tracking** (`f838295`, **ADR-074**) — `salida.json`, `salida.md`, `wf.json`, `temp_cells.txt` (salidas viejas de scripts/notebooks, no son fuente de nada) salen del repo y se agregan al `.gitignore`.

### Changed
- **Permisos mínimos en el workflow paper-live** (`ab68086`, **ADR-074**) — `paper_live_daily.yml` deja de pedir `actions:write` (ningún step lo usa); quedan `contents:write` (git push a `paper-live-data`) e `issues:write` (notify on failure). Menor privilegio = menor superficie si el token de CI se filtra.
...
### Added
- **Calidad de código en CI — ruff (bloqueante) + mypy (modo aviso)** (`544e067`, `01cddc9`, **ADR-074**) — `pyproject.toml` (nuevo) con config mínima. `ci.yml` corre **ruff** (pin `0.15.16`, frena el build) y **mypy** (pin `2.1.0`, `continue-on-error` → reporta sin frenar) sobre `core_sim` y `data`. Antes no existía linter ni type-checker. Limpiados los 75 hallazgos de ruff (imports/variables muertas, f-strings vacías, semicolons). mypy queda en adopción gradual (~55 hallazgos benignos: `float()/int()` sobre dicts de config tipados como `object`).
- **Cobertura del motor corto momentum en `run_paper_live`** (`6aff1bc`, **ADR-074**) — El sleeve corto momentum (`run_paper_live.py:391-414`) no se ejercía: producción corre en modo cobertura (`short_hedge.enabled=True`, ADR-064), que toma el otro branch. Nuevos tests con hedge apagado (corre el día completo) + branch "sin barras → saltear día". Cobertura de `run_paper_live` 83 → 89%. Tests: `tests/test_run_paper_live.py`.
- **Trigger de des-riesgo del largo por régimen VIX — research, behind flag** (**ADR-072/073**) — Sobre el cimiento de cash (ADR-071), un trigger que baja la exposición del largo en regímenes de miedo. `core_sim/long_regime_derisk.py`: funciones puras (gate de crash global de 4 condiciones + percentil rolling del `^VIX` sin look-ahead → exposición progresiva con histéresis). Cableado en `run_wf_research_sim.py` (flag off por defecto → producción idéntica). **Iteración 1** (VIX banda 70→95, criterio Sharpe-por-ventana) **NO PASÓ** (2/7 ventanas — el criterio por-ventana disfavorece estructuralmente a un de-risk, que sacrifica las calmas para proteger en el crash; **ADR-072**). **Iteración 2** (VIX banda 80→97, criterio Calmar agregado +5% con guardrail TWR) **PASÓ**: Calmar 2.316→3.276, drawdown -16.6→-10.5%, TWR 61.4→54.3% (**ADR-073**) → **candidata a promover** (pendiente: cablear `^VIX` en producción + robustez out-of-sample en otro período). El gate de 4 condiciones en pesos quedó descartado (abría 3/260 días: la devaluación tapa el crash medido en ARS). Tests: `tests/test_long_regime_derisk.py` (11). Producción y gate (ADR-041) intactos.
