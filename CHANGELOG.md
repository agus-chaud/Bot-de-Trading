...
- **Documento de complicaciones técnicas** (`docs/complicaciones-tecnicas.md`): consolida las 8 complicaciones técnicas del proyecto (síntoma, causa raíz, detección, resolución, estado y lección de cada una) como guion para defensa oral. Incluye el incidente IOL histórico 401, el bug de mapeo de keys del connector IOL (pendiente), el fallback Byma silencioso, el crash por hueco de datos (TXAR), la mezcla de monedas USD/ARS y la breadth insuficiente del universo.
